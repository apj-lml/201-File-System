...
@views.route('/to-menu/<emp_id>', methods=['GET', 'POST'])
@login_required
# @admin_permission.require(http_exception=403)
def travelOrder(emp_id):
    if request.method == 'GET':
        if str(current_user.id) != str(emp_id) and current_user.type_of_user == "user":
            return "YOU DO NOT HAVE ACCESS TO THIS PAGE! :P ", 404
        else:
            user = db.session.query(User).get(int(emp_id))
            users = db.session.query(User).options(db.defer('acknowledgement')).filter(User.status_remarks == 'ACTIVE').all()
            

            db.session.commit()

            return render_template('travel-order.html', emp_id = emp_id, user_profile = user, users=users)
