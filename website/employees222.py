...
def print_service_record(emp_id):
    ...
    if emp_service_record:
        ...
        # Extract values from each service record and add to the data list
        for record in emp_service_record:
            ...
            row = [service_from, service_to, designation, '', status]

            # Append the row to the data list
            data.append(row)

        # Insert the data at the specified starting row
        for i, row in enumerate(data, start=start_row):
            for j, value in enumerate(row, start=1):
                for row in data:
                    ...
                    for j, value in enumerate(row, start=1):
                        cell = sheet.cell(row=current_row_index, column=j)

                        if not cell.has_style:
                            for source_cell in prior_row:
